#!/usr/bin/env python3
"""Prepare 2-second focus_time recordings for Speech Commands training."""
...
def main() -> int:
    ...
    parser.add_argument("--label", default="focus_time")
    ...
    for path in target_dir.glob("eric_focus_time_*.wav"):
        path.unlink()

    sources = sorted(source_dir.glob("*.wav"))[: args.count]
    if len(sources) != args.count:
        raise RuntimeError(f"Expected {args.count} clips, found {len(sources)} in {source_dir}")

    split_plan = (["training"] * 24) + (["validation"] * 3) + (["testing"] * 3)
    if len(split_plan) != args.count:
        raise ValueError("The current split plan expects exactly 30 clips")

    counts = {"training": 0, "validation": 0, "testing": 0}
    for index, (source, target_set) in enumerate(zip(sources, split_plan), start=1):
        counts[target_set] += 1
        target_name = name_for_set("eric_focus_time", index, target_set)
